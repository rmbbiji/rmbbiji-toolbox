...
| `setup_zsh_tools_debian.sh` | Debian / Ubuntu 环境下一次安装 zsh、oh-my-zsh、`eza`、`bat`、`fd-find`、`zoxide` 和 Nerd Font，并安装/更新 `zsh-autosuggestions`、`zsh-syntax-highlighting`。脚本会备份已有 `.zshrc`，写入 `ls` / `ll` / `la` / `tree` / `cat` / `v` / `fd` / `zoxide` 相关配置，并为 `bat` 设置适合浅色/深色终端的默认主题，然后尝试切换当前用户默认 shell 为 zsh。 |
...
这个脚本面向 Debian / Ubuntu，会安装 zsh、oh-my-zsh、`eza`、`bat`、`fd-find`、`zoxide` 和 Nerd Font。执行完成后会自动进入新的 zsh 登录会话。
...
如果你想在一台新的 Debian / Ubuntu 服务器上直接使用这套配置，可以运行下面这一行。它会把 `.vimrc` 下载到 `~/.vimrc`，然后执行 `setup_zsh_tools_debian.sh` 安装 zsh / oh-my-zsh / `eza` / `bat` / `fd-find` / `zoxide` / Nerd Font。
