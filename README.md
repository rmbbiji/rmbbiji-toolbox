...
| `setup_zsh_tools_debian.sh` | Debian / Ubuntu 环境下一次安装 zsh、oh-my-zsh、Starship、`eza`、`bat`、`fd-find`、`zoxide` 和 Nerd Font，并安装/更新 `zsh-autosuggestions`、`zsh-syntax-highlighting`。脚本会备份已有 `.zshrc` 和 Starship 配置，写入 Catppuccin Powerline 双行提示符，以及 `ls` / `ll` / `la` / `tree` / `cat` / `v` / `fd` / `zoxide` 相关配置，其中 `cat` 会直接映射到 `bat`，并为 `bat` 设置兼容较旧 Debian 版本的默认主题，然后尝试切换当前用户默认 shell 为 zsh。 |
...
如果你想在一台新的 Debian / Ubuntu 服务器上直接使用这套配置，可以运行下面这一行。它会把 `.vimrc` 下载到 `~/.vimrc`，然后执行 `setup_zsh_tools_debian.sh` 安装 zsh / oh-my-zsh / Starship / `eza` / `bat` / `fd-find` / `zoxide` / Nerd Font。
