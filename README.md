...
| `setup_zsh_tools_debian.sh` | Debian / Ubuntu 环境下一次安装 zsh、oh-my-zsh、`eza`、`bat`、`fd-find`、`zoxide` 和 Nerd Font，并安装/更新 `zsh-autosuggestions`、`zsh-syntax-highlighting`。脚本会备份已有 `.zshrc`，写入 `ls` / `ll` / `la` / `tree` / `cat` / `fd` / `zoxide` 相关配置，并尝试切换当前用户默认 shell 为 zsh。 |
